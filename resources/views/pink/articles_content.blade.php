...
@foreach($articles as $article)
    ...
    <div class="sticky hentry hentry-post blog-big group">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In: <a href="{{ route('articlesCat', ['alias' => $article->category->alias]) }}" title="View all posts in {{ $article->category->title }}" rel="category tag">{{ $article->category->title }}</a></span></p>
            <p class="comments"><span><a href="article.html#respond" title="Comment on Section shortcodes &amp; sticky posts!">{{ count($article->comments) ?? '0' }} {{Lang::choice('ru.comments', count($article->comments))}}</a></span></p>
        </div>
        <!-- post content -->
...
{{--        {{ $articles->links('vendor.pagination.bootstrap-41', ['paginator' => $articles]) }}--}}
        {{ $articles->links('vendor.pagination.bootstrap-41') }}
    </div>
