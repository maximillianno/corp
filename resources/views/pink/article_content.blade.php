...
@if($article)
            ...
            <div class="meta group">
                <p class="author"><span>by <a href="blog-big-image.html" title="{{ $article->title }}" rel="author">{{ $article->user->name }}</a></span>
                <p class="categories"><span>In: <a href="{{ route('articlesCat', ['cat_alias' => $article->category->alias]) }}" title="View all posts in Happyness" rel="category tag">{{ $article->category->title }}</a>
...
@set($com, $article->comments->groupBy('parent_id'))
            ...
            <div id="respond">
                ...
                <form action="{{ route('comment.store') }}" method="post" id="commentform">
                    ...
                    <p class="comment-form-url"><label for="site">Website</label><input id="site" name="url" type="text" value="" size="30" /></p>
